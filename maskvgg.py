...
class MaskBlock(nn.Module):
    def __init__(self, in_channels, out_channels, args=None): 
        ...
        self.fc1 = nn.Linear(out_channels, out_channels // squeeze_rate, bias=False)
...
class MaskVGGBlock(BuildingBlock):
    ...
    def forward(self, x):
        x,_mask_list,_lasso_loss,_mask_before_list,_avg_fea_list=x       
        #_mask1 is the masked score, _lasso1 is score for each batch
       
        conv_out = self.conv(x)
        bn_out = self.batch_norm(conv_out)
        out = self.relu(bn_out)
        _masked_score_l,_lasso1, _score_l, _mask_l=self.mb1(out)                
        _mask_list.append(_masked_score_l)
        _lasso_loss.append(_lasso1)
        _mask_before_list.append(_score_l)    
        _avg_fea_list.append(F.adaptive_avg_pool2d(out,1))
        #out = out*_score_l.unsqueeze(-1).unsqueeze(-1)#only attention
        out=out* _masked_score_l.unsqueeze(-1).unsqueeze(-1)#attention for sarse
        #out=out* _mask_l.unsqueeze(-1).unsqueeze(-1)
    ...
    def do_pruning(self, in_channel_mask, out_channel_mask):
        ...
        with torch.no_grad():
            ...
            if mask_layer is not None:
                ...
                fc1_weight = fc1_weight[:, idx_out.tolist()]
                mask_layer.fc1.in_features = len(idx_out)
...
class VGG(nn.Module):
    ...
    def prune_model(self, x) -> None:
        ...
        for i in range(len(_mask_before_list)):
            ...
            maskl = torch.ge(total_mask_before , sco[int(sco.shape[0]*0.6)])
